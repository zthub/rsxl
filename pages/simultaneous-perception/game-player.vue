...
<script>
import { simultaneousGames } from './game-config.js'
export default {
  data() {
    return {
      instruction: '',
  ...
  onLoad(options) {
    ...
    const cfg = id && simultaneousGames[id] ? simultaneousGames[id] : null
    const title = cfg ? cfg.title : (options.title ? decodeURIComponent(options.title) : '同时知觉训练')
  ...
  onBackPress() {
    uni.redirectTo({ url: '/pages/simultaneous-perception/simultaneous-perception' })
    return true
  }
}
</script>
