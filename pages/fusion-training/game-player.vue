...
<script>
import { fusionGames } from './game-config.js'
export default {
  data() {
    return {
      instruction: '',
  ...
  onLoad(options) {
    ...
    const cfg = id && fusionGames[id] ? fusionGames[id] : null
    const title = cfg ? cfg.title : (options.title ? decodeURIComponent(options.title) : '融合训练')
    ...
    try {
      ...
      uni.setNavigationBarColor({ frontColor: '#ffffff', backgroundColor: '#96CEB4' })
    } catch (e) {}
  ...
  onBackPress() {
    uni.redirectTo({ url: '/pages/fusion-training/fusion-training' })
    return true
  }
}
</script>
